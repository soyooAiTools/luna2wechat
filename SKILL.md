...
**修法**：splash **只画 1 帧 + 1 次 RAF 触发 swap chain**，绝不持续 RAF loop。深色背景 + 半填进度条 + logo 已经是足够的"在加载"视觉反馈。
...
(function bootSplash() {
  ...
  if (!gl) return;
  // 同步画背景+进度条 1 次, 调 gl.flush() 提示提交, 再 1 次 RAF 触发 wx swap chain.
  // logo 异步 onload 后再画 1 次 + 1 次 RAF, 然后绝对不再画 (luna 接管时设 __lunaSplashStop).
})();
```

### splash + luna GL ctx 的禁忌（2026-05-06 升武器_unity 实战）

**铁律**: splash 跟 luna PC **共享同一个 GL ctx**（`g.canvas` 上的 webgl2/webgl）。任何 splash 在 luna 接管之后还在画的行为都会污染 luna 渲染状态：

| 错误做法 | 现象 | 根因 |
|---|---|---|
| `setInterval(50ms, drawSplash)` | luna init 拖 600ms-2.7s | 抢 GPU 队列, SKILL 原始坑 3 |
| **持续 RAF loop**（每帧都 redraw splash） | 画面素材**周边出黑方块** | splash GL state (BLEND/SCISSOR/viewport) 跟 luna draw 交错污染 |
| **RAF 不重设 viewport** | splash bar **画在中下偏右错位** | luna 改了 viewport, 我们的 scissor 按 luna viewport 映射 |
| splash stop 改在 setSource#10 (拖到 luna 已渲染几帧才停) | 同上, 重叠期 ~170ms | splash 跟 luna 重叠期 = 污染窗口 |

**正确做法**:
- splash 画 1 次 + 1 次 RAF swap (触发 wx 内核 present)
- logo onload 后画 1 次 + 1 次 RAF swap
- splash stop 信号设在 **setSource#1**（luna 第一次接管 GL 立刻停, 0 重叠）
- stop 之后所有 RAF callback 入口立即 `if (g.__lunaSplashStop) return` 不画

**变量名约定**: `GameGlobal.__lunaSplashStop`（双下划线）。曾经写过 `_splashStopped` 单下划线 → 跟 wx-ad-bridge 设的不一致 → 信号不通 → splash 永远不停 → 错位 bar 残留。统一用 `__lunaSplashStop`。

### loading 资源加载（manifest.json + logo PNG）

postprocess 提取 logo 后通过 `manifest.loadingLogos` 索引。first-screen 读 manifest 时**绝不能用 `require('./manifest.json')`**——试玩 runtime 不接 .json。必须：
...
逛 fs.readFileSync 是同步 API（不是 wx 上的 readFile），在 first-screen 这一层完全可用。

### Bar 位置坐标系（GL scissor）

GL scissor 坐标 **左下角原点**, Y 越大越靠上：

```js
const barY = Math.floor(H * 0.18);  // 距底部 18% = 屏幕下方靠下
// 不是 H * 0.62 (= 距底部 62% = 屏幕中上, 跟 logo 重叠位)
```

之前写过 `H * 0.62` 算反了被用户报"loading bar 跑在屏幕中上位置"，留作反面教材。
...
| 「loading bar 和 logo 都没了」启动期黑屏 | first-screen.js 用 `require('./manifest.json')` 读 logo 索引, 试玩 runtime 不接 .json → manifest=undefined → 永远走 "no logo entry" fallback; 同时 GL clear 单帧不会自动 swap | 用 `wx.getFileSystemManager().readFileSync('manifest.json', 'utf8') + JSON.parse`; GL 操作后调 1 次 RAF 触发 swap chain (不持续 RAF) |
| 「画面素材周边出黑方块」+「splash bar 在中下偏右错位」 | first-screen RAF 持续重绘, 跟 luna PC 共享同一 GL ctx → BLEND/SCISSOR/viewport 状态污染 luna draw | splash 改单帧 + 单 RAF swap; splash stop 信号设在 setSource#1 (luna 第一次接管立刻停, 0 重叠期); 变量名统一 `__lunaSplashStop` (双下划线), 不要写 `_splashStopped` |
